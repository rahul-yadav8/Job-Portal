...
const LocalCalendar: React.FC<Props> = ({ value, onChange, minDate }) => {
  ...
  return (
    <div className='local-calendar w-[300px] rounded  p-4 font-sans'>
      <div className='mb-4 flex items-center justify-between'>
        ...
        <div className='justify-start text-sm font-semibold leading-tight tracking-tight text-neutral-800'>
          {monthNames[currentMonth]} {currentYear}
        </div>
        <button type='button' onClick={handleNext} aria-label='Next Month'>
          <Flex className='items-center justify-center'>
            <svg xmlns='http://www.w3.org/2000/svg' width='29' height='28' viewBox='0 0 29 28' fill='none'>
              <circle cx='14.5152' cy='13.773' r='13.773' fill='#3b82f6' opacity={'0.2'} />
              <path
      ...
      <div className='mb-2 grid grid-cols-7 gap-1 text-center text-sm font-medium text-gray-500'>
        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((d) => (
          <div key={d} className='justify-center text-center text-xs font-semibold leading-3 text-zinc-600'>
            {d}
          </div>
        ))}
      </div>
        ...
        {days.map((date) => {
          ...
          return (
            <button
              ...
              className={`h-8 w-8 cursor-pointer rounded-full py-2 text-xs  ${
                disabled
